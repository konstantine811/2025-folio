...
export const buildTile = ({
  ...
  buildContext.log(
    ...
    `Building tile at x: ${tileX}, y: ${tileY}`
  );
  buildContext.log(
    Recast.RC_LOG_PROGRESS,
  ...
  const tileBounds = new Box3(
    new Vector3(
      ...
      expandedTileBoundsMin[1],
      expandedTileBoundsMin[2]
    ),
    new Vector3(
      expandedTileBoundsMax[0],
      expandedTileBoundsMax[1],
      expandedTileBoundsMax[2]
    )
  );

  const trianglesInTileBounds = getTrianglesInBox(
    positions,
    indices,
    tileBounds
  );
  ...
  navMeshCreateParams.setWalkableHeight(
    recastConfig.walkableHeight * recastConfig.ch
  );
  navMeshCreateParams.setWalkableRadius(
    recastConfig.walkableRadius * recastConfig.cs
  );
  navMeshCreateParams.setWalkableClimb(
    recastConfig.walkableClimb * recastConfig.ch
  );
