import { shaderMaterial } from "@react-three/drei";
import { extend } from "@react-three/fiber";
import { Vector2 } from "three";

export const WinderMaterial = shaderMaterial(
  {
    // текстури / прозорість
    albedoMap: null,
    edgeMaskMap: null,
    useEdgeMask: 10.0,
    transparency: 0.0,

    // ВІТЕР (залишаємо лише його)
    time: 0.0,
    uWindAmp: 0.06,
    uWindFreq: 1.2,
    uWindDir: new Vector2(0.85, 0.2).normalize(),

    // деталі вітру (за бажанням можеш тюнити або прибрати)
    windDirNoiseScale: 0.05,
    windStrNoiseScale: 0.25,
    gustStrength: 1.25,
    noiseScrollDir: 0.05,

    // fallback затемнення країв, якщо edgeMask нема
    _fallbackEdgeWidth: 1.05,
    _fallbackEdgeDark: 0.01,
  },
  /* glsl */ `
  // ===== attributes =====
  attribute float aBend;
  attribute float aTip;
  attribute float aPhase;

  // ===== uniforms =====
  uniform float time;
  uniform float uWindAmp;
  uniform float uWindFreq;
  uniform vec2  uWindDir;

  // varyings
  varying vec2 vUv;
  varying vec3 vWorldPos;
  varying vec3 vNormalW;

  mat3 mat3fromMat4(mat4 m){ return mat3(m[0].xyz, m[1].xyz, m[2].xyz); }

  void main() {
    vUv = uv;

    // локаль
    vec3 pos = position;
    vec3 nrm = normal;

    // БАЗОВЕ КОЛИХАННЯ ВІТРОМ (без presence/touch, без "tall" логіки)
    float t = time * uWindFreq;
    float phase = aPhase * 6.2831853;  // 2π
    float wave  = sin(t + phase);
    float gust  = sin(t * 0.37 + phase * 1.7);

    float tipWeight = smoothstep(0.5, 1.0, aTip);
    tipWeight *= tipWeight;

    float amp = uWindAmp * aBend * (0.7 + 0.3 * gust);
    vec2 disp = normalize(uWindDir) * (wave * amp) * tipWeight;
    pos.xz += disp;

    // у world-позицію
    #ifdef USE_INSTANCING
      vec4 wp = modelMatrix * instanceMatrix * vec4(pos, 1.0);
      mat3 nm = mat3fromMat4(modelMatrix * instanceMatrix);
    #else
      vec4 wp = modelMatrix * vec4(pos, 1.0);
      mat3 nm = mat3fromMat4(modelMatrix);
    #endif

    vWorldPos = wp.xyz;
    vNormalW  = normalize(nm * nrm);
    gl_Position = projectionMatrix * viewMatrix * wp;
  }
  `,
  /* glsl */ `
  uniform sampler2D albedoMap;
  uniform sampler2D edgeMaskMap;
  uniform float useEdgeMask;
  uniform float transparency;

  uniform float _fallbackEdgeWidth;
  uniform float _fallbackEdgeDark;

  varying vec2 vUv;
  varying vec3 vWorldPos;
  varying vec3 vNormalW;

  void main() {
    // базовий колір
    vec3 col = texture2D(albedoMap, vUv).rgb;

    // затемнення країв
    float edge = 0.0;
    if (useEdgeMask > 0.5) {
      float m = texture2D(edgeMaskMap, vUv).r; // білий центр, чорні краї
      edge = 1.0 - m;
    } else {
      float d = min(min(vUv.x, 1.0 - vUv.x), min(vUv.y, 1.0 - vUv.y));
      edge = 1.0 - smoothstep(0.0, max(1e-4, _fallbackEdgeWidth), d);
    }
    float dark = mix(1.0, _fallbackEdgeDark, edge);
    col *= dark;

    gl_FragColor = vec4(clamp(col, 0.0, 1.0), 1.0 - transparency);
  }
  `
);
extend({ WinderMaterial });
