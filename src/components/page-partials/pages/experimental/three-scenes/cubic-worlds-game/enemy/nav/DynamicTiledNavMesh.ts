...
import DynamicTiledNavMeshWorker from "@/workers/three/build-tile.worker?worker";
import { Box3, Vector3 } from "three";
import { BuildTileMeshProps } from "@/lib/three/tile/build-tile";
...
export class DynamicTiledNavMesh {
  ...
  onNavMeshUpdate = new Topic<
    [version: number, tile: [x: number, y: number]]
  >();

  navMeshBounds: [min: Vector3Tuple, max: Vector3Tuple];
  navMeshBoundsMin: Vector3;
  navMeshBoundsMax: Vector3;
  navMeshOrigin: Vector3;
  ...
  constructor(props: DynamicTiledNavMeshProps) {
    ...
    const navMeshBounds: [min: Vector3Tuple, max: Vector3Tuple] = [
      navMeshBoundsMin.toArray(),
      navMeshBoundsMax.toArray(),
    ];
    ...
    for (let i = 0; i < props.workers; i++) {
      ...
      worker.onmessage = (e) => {
        ...
        navMeshData.copy(
          serialisedNavMeshData as ArrayLike<number> as number[]
        );
        ...
        if (statusFailed(addTileResult.status)) {
          console.error(
            `Failed to add tile to nav mesh at [${tileX}, ${tileY}], status: ${
              addTileResult.status
            } (${statusToReadableString(addTileResult.status)}`
          );
  ...
  getTileForWorldPosition(worldPosition: Vector3) {
    const x = Math.floor(
      (worldPosition.x - this.navMeshBoundsMin.x) / this.tcs
    );
    const y = Math.floor(
      (worldPosition.z - this.navMeshBoundsMin.z) / this.tcs
    );

    return [x, y] as [x: number, y: number];
  }

  getTilesForBounds(bounds: Box3) {
