...
const UStudioSection = () => {
  ...
  const dskOpacity = [0.6, 0.62, 0.77, 0.78];
  const dskScale = [-600, -500];
  let infoScale = dskScale;
  let infoOpacity = dskOpacity;
  if (isLargeSize && !isMdSize) {
    infoOpacity = [0.6, 0.62, 0.77, 0.78];
    infoScale = [-300, -200];
  } else if (isMdSize) {
    infoOpacity = [0.5, 0.62, 0.8, 0.88];
    infoScale = [-100, -400];
  } else {
  ...
  const infoSkillY = useTransform(p, [0.75, 0.85], infoScale);
  return (
    <motion.div
      className="flex flex-col justify-between gap-4 md:gap-10"
      style={{ y: infoSkillY }}
    >
      <div className="flex flex-col  gap-4 md:gap-10 md:px-10 lg:px-20 border-t-1 border-muted-foreground/30 mt-30 pt-20">
            ...
            <div className="lg:w-3/4 flex flex-col gap-10">
              ...
              <div className="text-foreground/70 text-md lg:text-xl font-pixel flex flex-col gap-4">
                <div className="flex flex-col gap-4">
                  <Trans
                    i18nKey="cubic_worlds_game.portfolio.experience.u_studio"
