...
const SeventhSection = () => {
  ...
  const dskOpacity = [0.45, 0.5, 0.51, 0.55];
  const dskScale = [-500, -1000];
  let infoScale = dskScale;
  let infoOpacity = dskOpacity;
  if (isLargeSize && !isMdSize) {
    infoOpacity = [0.45, 0.5, 0.51, 0.55];
    infoScale = [-200, -500];
  } else if (isMdSize) {
    infoOpacity = [0.4, 0.45, 0.51, 0.65];
    infoScale = [0, 0];
  } else {
  ...
  const infoSkillY = useTransform(p, [0.45, 0.55], infoScale);
  return (
    <motion.div
            ...
            <div className="lg:w-3/4 flex flex-col gap-10 ">
              ...
              <div className="text-foreground/70 text-md lg:text-xl font-pixel flex flex-col gap-4">
                <div className="flex flex-col gap-4">
                  <Trans
                    i18nKey="cubic_worlds_game.portfolio.skills_section.ui"
