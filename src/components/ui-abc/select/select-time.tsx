...
const ScrollColumn = ({
  ...
  return (
    <div
      ref={ref}
      className="h-[84px] w-16 overflow-y-auto snap-y snap-mandatory scroll-smooth no-scrollbar"
    >
      {values.map((val) => {
        const isValidValue =
          val !== "-1" &&
        ...
        return (
          <div
            key={val}
            className={`snap-center flex items-center justify-center text-xl transition-colors ${
              val === selected
  ...
  return (
    ...
    >
      <div className="relative flex justify-center space-x-2">
        ...
        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-7 border-y border-foreground/30  pointer-events-none" />
        <ScrollColumn
          type="hour"
          values={range(26)}
          selected={hour}
